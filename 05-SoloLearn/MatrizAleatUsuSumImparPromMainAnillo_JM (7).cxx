...
Y al final elejir el nro de anillo a imprimir */
#include <iostream>
#include <conio.h>
#include <stdlib.h>
#include <time.h>
...
int main ()
{
    ...
    while (NumAnillo>=((fil/2)-1)  && NumAnillo>=((col/2)-1) );
...
{
int i,j;
  // if (NumAnillo<=((fil/2)-1)  && NumAnillo<=((col/2)-1) ){
    for (i=0; i<fil; i++)
    {
        for (j=0; j<col; j++){
            if( (i==NumAnillo && j>=NumAnillo) && j<=(col-1)-NumAnillo
           ||   i==(fil-1)-NumAnillo&& j>=NumAnillo && j<=(col-1)-NumAnillo
            ||   j==NumAnillo && i<=(fil-1)-NumAnillo && i>=NumAnillo
            ||   j==(col-1)-NumAnillo && i>=NumAnillo && i<=(fil-1)-NumAnillo
            )
            {
                cout<<setw(4)<<ma[i][j];
            }       
